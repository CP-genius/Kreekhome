...
function createNativeAdSlot() {
    ...
    // Load ad script after a short delay
    setTimeout(() => {
        if (!document.querySelector(`script[src="${NATIVE_AD_CONFIG.scriptSrc}"]`)) {
...
function insertNativeAds(container, cards, cardType) {
    ...
    cards.forEach((card, index) => {
        ...
        // Insert native ad after every N cards
        if ((index + 1) % NATIVE_AD_CONFIG.adInterval === 0 && index !== cards.length - 1) {
