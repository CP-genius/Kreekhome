...
function goToNextQuestion() {
    ...
    if (isLastQuestion) {
        ...
        if (hasAnswer) {
            // Show toast and go to next question
...
function checkAnswer(questionIndex) {
    ...
    // Show toast
    if (isCorrect) {
        showToast(correctToast, "Correct! Well done.");
    } else {
        showToast(incorrectToast, "Incorrect. Try again!");
    }
}
...
function showInterstitialAd() {
    ...
    // Update ad container with simulated ad
    adContainer.innerHTML = `
        <div class="ad-label">Advertisement</div>
        <div style="text-align: center; padding: 20px;">
            <h4 style="color: var(--primary); margin-bottom: 10px;">Special Offer!</h4>
            <p style="color: var(--gray); margin-bottom: 15px;">Get premium features with our educational partners</p>
            <button style="background: var(--primary); color: white; border: none; padding: 10px 20px; border-radius: 6px; cursor: pointer;">
                Learn More
            </button>
        </div>
    `;
...
// Load topics for a subject - UPDATED: Remove "All Topics" for topic-based subjects
...
// Load quiz numbers for a subject and topic - UPDATED
